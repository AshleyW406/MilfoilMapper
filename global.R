...
MSU_database <- read.csv("MSU_PlantDatabase_033126.csv")
...
MSU_database = MSU_database %>% 
  mutate(strain_response = paste0("<span id=arrowtag>&#x2794 </span>", Microsatellite_strain, 
                                  " (2,4-D: " , 
                                  str_to_title(X2_4D_response), 
                                  "; Fluridone: ", 
                                  str_to_title(Fluridone_response), 
                                  "; Procellacor: ",
                                  str_to_title(Florpyrauxifen_benzyl_response),
                                  ")" ))
  


#A FOR LOOP CREATING AND POPULATING THE RESISTANT AND SUSCEPTIBLE BUTTONS

for(i in 1:nrow(MSU_database)) {
  
  MSU_database$strain_response[i] = gsub(pattern = "Fluridone: Susceptible",
                                         replacement = paste0("Fluridone: ",
                                                              "<button class = 'response-button' data-id = '",
                                                              MSU_database$Microsatellite_strain[i], "-Fluridone", 
                                                              "'><span class = 'color_green'>Susceptible</span></button>"), 
  ...
  MSU_database$strain_response[i] = gsub(pattern = "2,4-D: Susceptible",
  ...
  MSU_database$strain_response[i] = gsub(pattern = "2,4-D: Of Concern",
                                         replacement = paste0("2,4-D: ",
                                                              "<button class = 'response-button' data-id = '",
                                                              MSU_database$Microsatellite_strain[i], "-2,4-D",
                                                              "'><span class = 'color_orange'>Of Concern</span></button>"), 
  ...
  MSU_database$strain_response[i] = gsub(pattern = "Procellacor: Susceptible",
                                         replacement = paste0("Procellacor: ",
                                                              "<button class = 'response-button' data-id = '",
                                                              MSU_database$Microsatellite_strain[i], "-Procellacor", 
                                                              "'><span class = 'color_green'>Susceptible</span></button>"), 
                                         x = MSU_database$strain_response[i])
  
  MSU_database$strain_response[i] = gsub(pattern = "Procellacor: Resistant",
                                         replacement = paste0("Procellacor: ",
                                                              "<button class = 'response-button' data-id = '",
                                                              MSU_database$Microsatellite_strain[i], "-Procellacor",
                                                              "'><span class = 'color_pink'>Resistant</span></button>"), 
  ...
  MSU_database$strain_response[i] = gsub(pattern = "Procellacor: Of Concern",
                                         replacement = paste0("Procellacor: ",
                                                              "<button class = 'response-button' data-id = '",
                                                              MSU_database$Microsatellite_strain[i], "-Procellacor",
                                                              "'><span class = 'color_orange'>Of Concern</span></button>"), 
...
state_codes <- c("All" = "All", "Colorado" = "CO", "Idaho" = "ID", "Illinois" = "IL", 
                 "Indiana" = "IN", "Iowa" = "IA", "Massachusetts" = "MA", 
                 "Maryland" = "MD", "Michigan" = "MI", "Minnesota" = "MN",
                  "Montana" = "MT",  "Nebraska" = "NE", "New York" = "NY",
                  "Ohio" = "OH", "Pennsylvania" = "PA", "South Carolina" = "SC", 
...
#GETTING RID OF (NA) IF THERE IS NOT A YEAR
for(r in 1:nrow(MSU_db_markers)) {
  years <- unlist(strsplit(MSU_db_markers$Year_collected[r], ",\\s*"))
  years <- trimws(years)  # remove extra spaces
  
  # only remove "NA" if there are other values present
  if (length(years[years != "NA" & years != ""]) > 0) {
    years <- years[years != "NA" & years != ""]
  }
  
  # collapse back to a single string
  if (length(years) == 0) {
    MSU_db_markers$Year_collected[r] <- "NA"
  } else {
    MSU_db_markers$Year_collected[r] <- paste(years, collapse = ", ")
  }
}
...
MSU_db_markers_sb$maplabels = lapply(seq(1, nrow(MSU_db_markers_sb)),
                                  function(i) {
                                    paste0("<span class = 'bold_this'> State: </span>", MSU_db_markers_sb[i, "State"], '<br>',
                                           ...
                                           "<span class = 'bold_this'> Year(s) Collected: </span>", MSU_db_markers_sb[i, "Year_collected"], '<br>',
